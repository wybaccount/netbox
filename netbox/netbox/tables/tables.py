...
class BaseTable(tables.Table):
    ...
    def _get_columns(self, visible=True):
        ...
        columnsMap = {
            ...
            'Prefix (Flat)': '前缀（扁平）',
            'Assigned': '已分配',
            'NAT (Inside)': '网络地址转换（内部）',
            'NAT (Outside)': '网络地址转换（外部）',
            'Interface': '接口',
            'Device/VM': '设备/虚拟机',
            'Part ID': '零件编号',
            'FHRP Groups': '第一跳冗余协议组',
            'Untagged VLAN': '未标记的虚拟局域网',
            'Tagged VLANs': '已标记的虚拟局域网',
            'Parent Device': '父设备',
            'Position (Device Bay)': '位置（设备插槽）',
            'VC Position': '虚拟通道位置',
            'VC Priority': '虚拟通道优先级',
            'Facility ID': '设备编号',
            'Last updated': '最后更新时间',
            ...
            'Import targets': '导入目标',
            'Export targets': '导出目标',
            'Minimum VLAN ID': '最小VLAN编号',
            'Maximum VLAN ID': '最大VLAN编号',
            'Scope type': '范围类型',
            'Scope': '范围',
